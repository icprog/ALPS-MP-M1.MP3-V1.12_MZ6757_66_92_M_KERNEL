...
static int msdc_debug_proc_show(struct seq_file *m, void *v)
{
	...
	if (cmd == SD_TOOL_ZONE) {
		...
		} else if ((p2 == 0 && (unsigned int)p3 > 0xfffff)
			|| (p2 == 1 && (unsigned int)p3 > 0xfffff)) {
			seq_puts(m, "[SD_Debug]Some rd/td value was invalid (rd mask:(0xfffff),td mask:(0xfffff))\n");
		} else {
			if (p2 == 0) {
		...
		if (p1 == 0) {
			...
			MSDC_SET_FIELD(MSDC_PAD_TUNE0, MSDC_PAD_TUNE0_CMDRDLY, p2);
		...
		seq_printf(m, " fix vcore: %d\n\n",
			host->fix_vcore);
		seq_puts(m, "   retry        count\n");
		seq_puts(m, "=======================\n");
		for (size = 1; size < (host->dma_max_retry_cnt + 1); size++) {
			seq_printf(m, "    %d:        %8d\n",
				size + 1, host->dma_retry_stat[size]);
		}
#else
