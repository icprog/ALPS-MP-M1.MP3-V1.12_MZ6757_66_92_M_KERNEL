...
static int of_get_synaptic_platform_data(struct device *dev)
{
	...
	pr_warn("tpd_rst_gpio_number %d\n", tpd_rst_gpio_number);
	pr_warn("tpd_int_gpio_number %d\n", tpd_int_gpio_number);
	return 0;
}
#else
static int of_get_synaptic_platform_data(struct device *dev)
...
static int tpd_irq_registration(void)
{
	...
	if (node) {
		...
		if (ret > 0) {
			pr_err("tpd request_irq IRQ LINE NOT AVAILABLE!.");
			return -1;
		}
	} else {
		pr_err("tpd request_irq can not find touch eint device node!.");
		return -1;
	}
				...
				unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	msg[0].flags = 0;
	msg[0].len = length + 1;
	msg[0].ext_flags = (rmi4_data->i2c_client->ext_flag | I2C_ENEXT_FLAG | I2C_DMA_FLAG),
		...
		unsigned int intr_count)
{
	...
	fhandler->num_of_data_points = min_t(unsigned char, ctrl_23.max_reported_objects,
...
/*#endif*/
{
	...
	if (tpd->tpd_dev->of_node) {
		pr_warn("of_find_compatible_node-->mediatek,mt6797-touch\n");
		...
		pr_err("of_find_compatible_node-->mediatek,mt6797-touch,error\n");

#endif /* add 3.3v end */
#endif

#ifdef USE_I2C_DMA
	wDMABuf_va = (unsigned char *)dma_zalloc_coherent(NULL, WRITE_SIZE_LIMIT,
...
static struct i2c_board_info i2c_tpd __initdata = { I2C_BOARD_INFO("synaptics_dsx", (TPD_I2C_ADDR))};
...
#ifdef USE_I2C_DMA
	if (wDMABuf_va)
		dma_free_coherent(NULL, WRITE_SIZE_LIMIT , wDMABuf_va, wDMABuf_pa);
#endif
