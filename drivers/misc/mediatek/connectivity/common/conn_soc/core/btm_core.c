	...
	case STP_OPID_BTM_PAGED_DUMP:
		...
		do {
			...
			while (1) {
				...
				STP_BTM_DBG_FUNC("waiting chip put done\n");
			...
			STP_BTM_DBG_FUNC("dump_phy_ddr(%08x),dump_vir_add(0x%p),dump_len(%d)\n",
...
#if 1				/* Paged dump */
	STP_BTM_DBG_FUNC("paged dump start++\n");
	retval = _stp_btm_dump_type(stp_btm, STP_OPID_BTM_PAGED_DUMP);
	if (retval)
		STP_BTM_DBG_FUNC("paged dump fail\n");
#else
