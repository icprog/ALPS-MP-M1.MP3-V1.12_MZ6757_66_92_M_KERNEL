...
/*
 * check if driver/firmware env is ok for
 * probe response TX when device is in
 * listen state
 * return:
 *  0: OK for TX
 *  -1: should drop
 */
static int
p2pFuncPbrTxBedCheck(IN P_ADAPTER_T prAdapter, IN P_MSDU_INFO_T prMgmtTxMsdu)
{
	P_WLAN_PROBE_RSP_FRAME_T prProbRspHdr = (P_WLAN_PROBE_RSP_FRAME_T)NULL;
	...
	if (u2CapInfo & CAP_INFO_BSS_TYPE)
		return 0;
	/*
	 * this probe response is TX by a p2p device which is being in
	 * listen state, should check if channel is on hand to TX
	 */
	switch (prP2pFsmInfo->eCurrentState) {
	case P2P_STATE_CHNL_ON_HAND:
		return 0;
	default:
		DBGLOG(P2P, WARN, "Should drop probe response due to channel not on hand\n");
	}
	return -1;
}
		   ...
		   IN P_P2P_MGMT_TX_REQ_INFO_T prMgmtTxReqInfo, IN P_MSDU_INFO_T prMgmtTxMsdu, IN UINT_64 u8Cookie)
{
	...
	do {
		...
		case MAC_FRAME_PROBE_RSP:
			...
			if (p2pFuncPbrTxBedCheck(prAdapter, prMgmtTxMsdu)) {
		...
		if (fgDrop) {
			p2pFsmRunEventMgmtFrameTxDone(prAdapter, prMgmtTxMsdu,
