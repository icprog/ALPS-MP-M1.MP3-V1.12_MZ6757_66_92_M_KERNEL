				  ...
				  IN UINT_8 aucSrcAddr[], IN BOOLEAN fgCheckSsid, IN P_PARAM_SSID_T prSsid)
{
	...
	case BSS_TYPE_P2P_DEVICE:
		/*
		 * should zero SSID in case: GO --> P2P DEVICE
		 * with the same BSSID
		 */
		/* fgCheckSsid = FALSE; */
...
P_BSS_DESC_T scanAddToBssDesc(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb)
{
	...
	case 0:
		/*
		 * The P2P Device in listen state shall set the ESS bit of the Capabilities field
	...
	IE_FOR_EACH(pucIE, u2IELength, u2Offset) {
		...
		case ELEM_ID_SSID:
			if ((!prIeSsid) &&	/* NOTE(Kevin): for Atheros IOT #1 */
				...
				/*
				 * should zero SSID in case:
				 * GO --> P2P DEVICE
				 */
				if (eBSSType == BSS_TYPE_P2P_DEVICE && prBssDesc)
					kalMemZero(prBssDesc->aucSSID, ELEM_MAX_LEN_SSID);
