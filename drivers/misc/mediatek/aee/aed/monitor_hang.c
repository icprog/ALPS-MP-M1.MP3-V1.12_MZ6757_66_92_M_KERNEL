...
static int FindTaskByName(char *name)
{
	...
	for_each_process(task) {
		...
		} else if (task && (strstr(task->comm, "logd"))) {
			logd_pid = task->pid;
			LOGE("[Hang_Detect] %s found pid:%d.\n", task->comm, task->pid);
		}
	 }
...
static void ShowStatus(void)
{
	...
	/* show all kbt in surfaceflinger */
	LOGE("[Hang_Detect] dump logd all thread bt\n");
	if (logd_pid)
		show_bt_by_pid(logd_pid);
	...
	LOGE("[Hang_Detect] dump msdc status\n");
	msdc_dump_status();
	LOGE("[Hang_Detect] dump msdc status end\n");
