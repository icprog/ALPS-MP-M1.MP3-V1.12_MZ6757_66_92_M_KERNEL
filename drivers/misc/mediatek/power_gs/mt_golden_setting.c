...
#else
		*((unsigned int *)IO_PHYS_TO_VIRT(addr)) = (*((unsigned int *)IO_PHYS_TO_VIRT(addr)) & ~mask) |
			(reg_val & mask);
#endif
	}
}
...
static int golden_test_proc_show(struct seq_file *m, void *v)
{
	...
	if (0 == _golden.nr_golden_setting) {
		...
		while ((0 != idx) || (NULL != (snapshot = _snapshot_consume(&_golden)))) {
			if (0 == idx)
