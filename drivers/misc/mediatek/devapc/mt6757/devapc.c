...
#if 1 /* defined(CONFIG_MTK_AEE_FEATURE) && defined(DEVAPC_ENABLE_AEE) */
#include "aee.h"
#endif
...
static void check_vio_is_mcucfg(unsigned int i,
				unsigned int dbg0,
				unsigned int dbg1)
{
	char aee_str[256];
	char const str[] = "INFRASYS_MCUSYS_CONFIG_REG";

	if (!strncmp(devapc_devices[i].device, str, strlen(str))) {
		snprintf(aee_str, 256,
...
static irqreturn_t devapc_violation_irq(int irq, void *dev_id)
{
	...
	for (i = 0; i < device_count; i++) {
		...
		if ((check_vio_status(i)) && (devapc_devices[i].enable_vio_irq == true)) {
			...
			check_vio_is_mcucfg(i, dbg0, dbg1);
