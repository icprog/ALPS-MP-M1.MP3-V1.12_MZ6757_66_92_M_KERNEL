...
static const u32 rt9468_dc_vbatov_lvl[RT9468_DC_VBATOV_LVL_NUM] = {
	104, 108, 119,
}; /* % * VOREG */
...
static bool rt9468_is_hw_exist(void)
{
	...
	battery_log(BAT_LOG_CRTI, "%s: rt9468 single revision\n", __func__);
	return true;
}
...
static int rt9468_enable_pump_express(u8 enable)
{
	...
	ret = (enable ? rt9468_set_bit(RT9468_REG_CHG_CTRL17, RT9468_MASK_PUMPX_EN)
		: rt9468_clr_bit(RT9468_REG_CHG_CTRL17, RT9468_MASK_PUMPX_EN));
	if (ret < 0)
		return ret;

	for (i = 0; i < retry_times; i++) {
		msleep(2500);
		if (rt9468_i2c_test_bit(RT9468_REG_CHG_CTRL17, RT9468_SHIFT_PUMPX_EN) == 0)
			break;
		if ((i + 1) == retry_times) {
...
static int rt9468_get_battery_voreg(u32 *voreg)
{
	int ret = 0;
	u8 reg_voreg = 0;

	ret = rt9468_i2c_read_byte(RT9468_REG_CHG_CTRL4);
	if (ret < 0)
		return ret;
...
static int rt_charger_enable_direct_charge(void *data)
{
	...
	if (enable) {
		...
		/* VG_EN = 1 */
		ret = rt9468_set_bit(RT9468_REG_CHG_PUMP, RT9468_MASK_VG_EN);
		if (ret < 0)
...
static int rt_charger_set_pep_current_pattern(void *data)
{
	...
	/* Set Pump Up/Down */
	ret = (is_pump_up ?
		rt9468_set_bit(RT9468_REG_CHG_CTRL17, RT9468_MASK_PUMPX_UP_DN)
		: rt9468_clr_bit(RT9468_REG_CHG_CTRL17, RT9468_MASK_PUMPX_UP_DN));
	if (ret < 0)
		return ret;
...
static int rt_charger_set_pep20_current_pattern(void *data)
{
	...
	/* Set to PEP2.0 */
	ret = rt9468_set_bit(RT9468_REG_CHG_CTRL17, RT9468_MASK_PUMPX_20_10);
	if (ret < 0)
		return ret;

	/* Set Voltage */
	ret = rt9468_i2c_update_bits(RT9468_REG_CHG_CTRL17,
...
static int rt_charger_reset_dc_watchdog_timer(void *data)
{
	/* Any I2C communication can reset watchdog timer */
	int ret = 0;
	enum rt9468_charging_status chg_status;
...
static int rt_charger_enable_dc_vbusov(void *data)
{
	int ret = 0;
	bool enable = *((bool *)data);

	ret = (enable ? rt9468_set_bit(RT9468_REG_CHG_DIRCHG3, RT9468_MASK_DC_VBUSOV_EN)
		: rt9468_clr_bit(RT9468_REG_CHG_DIRCHG3, RT9468_MASK_DC_VBUSOV_EN));
	return ret;
}

static int rt_charger_set_dc_vbusov(void *data)
{
	int ret = 0;
	u8 reg_vbusov = 0;
	u32 vbusov = *((u32 *)data);
...
static int rt_charger_enable_dc_vbusoc(void *data)
{
	int ret = 0;
	bool enable = *((bool *)data);

	ret = (enable ? rt9468_set_bit(RT9468_REG_CHG_DIRCHG1, RT9468_MASK_DC_VBUSOC_EN)
		: rt9468_clr_bit(RT9468_REG_CHG_DIRCHG1, RT9468_MASK_DC_VBUSOC_EN));
	return ret;
}

static int rt_charger_set_dc_vbusoc(void *data)
{
	int ret = 0;
	u8 reg_vbusoc = 0;
	u32 vbusoc = *((u32 *)data);
...
static int rt_charger_enable_dc_vbatov(void *data)
{
	int ret = 0;
	bool enable = *((bool *)data);

	ret = (enable ? rt9468_set_bit(RT9468_REG_CHG_DIRCHG1, RT9468_MASK_DC_VBATOV_EN)
		: rt9468_clr_bit(RT9468_REG_CHG_DIRCHG1, RT9468_MASK_DC_VBATOV_EN));
	return ret;
}

static int rt_charger_set_dc_vbatov(void *data)
{
	int ret = 0, i = 0;
	u8 reg_vbatov = 0;
	...
	ret = rt9468_get_battery_voreg(&voreg);
	if (ret < 0) {
		battery_log(BAT_LOG_CRTI, "%s: get voreg failed\n", __func__);
		return ret;
	}

	for (i = 0; i < RT9468_DC_VBATOV_LVL_NUM; i++) {
		...
		if ((i + 1) == RT9468_DC_VBATOV_LVL_NUM)
			reg_vbatov = i;
	}
...
static int rt_charger_is_dc_enable(void *data)
{
	int ret = 0;

	ret = rt9468_i2c_test_bit(RT9468_REG_CHG_PUMP, RT9468_SHIFT_VG_EN);
	if (ret < 0)
		return ret;
...
static void rt9468_charger_intf_init(void)
{
	...
	mtk_charger_intf[CHARGING_CMD_RESET_DC_WATCH_DOG_TIMER] = rt_charger_reset_dc_watchdog_timer;
	mtk_charger_intf[CHARGING_CMD_ENABLE_DC_VBUSOV] = rt_charger_enable_dc_vbusov;
	mtk_charger_intf[CHARGING_CMD_SET_DC_VBUSOV] = rt_charger_set_dc_vbusov;
	mtk_charger_intf[CHARGING_CMD_ENABLE_DC_VBUSOC] = rt_charger_enable_dc_vbusoc;
	mtk_charger_intf[CHARGING_CMD_SET_DC_VBUSOC] = rt_charger_set_dc_vbusoc;
	mtk_charger_intf[CHARGING_CMD_ENABLE_DC_VBATOV] = rt_charger_enable_dc_vbatov;
	mtk_charger_intf[CHARGING_CMD_SET_DC_VBATOV] = rt_charger_set_dc_vbatov;
	mtk_charger_intf[CHARGING_CMD_GET_IS_DC_ENABLE] = rt_charger_is_dc_enable;
