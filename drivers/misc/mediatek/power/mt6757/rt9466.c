...
static int rt9466_i2c_block_write(u8 cmd, u32 leng, const u8 *data)
{
	...
	ret = rt9466_regmap_write(g_rt9466_info.i2c, cmd, leng, data);
	return ret;
}
...
static int rt9466_get_adc(enum rt9466_adc_sel adc_sel, int *adc_val)
{
	...
	for (i = 0; i < wait_retry; i++) {
		mdelay(35);
		if (rt9466_i2c_test_bit(RT9466_REG_CHG_ADC, RT9466_SHIFT_ADC_START) == 0) {
...
static int rt9466_enable_jeita(bool enable)
{
	...
	ret = (enable ? rt9466_set_bit(RT9466_REG_CHG_CTRL16, RT9466_MASK_JEITA_EN)
		: rt9466_clr_bit(RT9466_REG_CHG_CTRL16, RT9466_MASK_JEITA_EN));
	return ret;
}
...
static int rt_charger_enable_otg(void *data)
{
	...
	for (i = 0; i < ss_wait_times; i++) {
		...
		if ((i + 1) == ss_wait_times) {
			...
			/* Disable OTG */
			rt9466_clr_bit(RT9466_REG_CHG_CTRL1,
...
static int rt_charger_is_power_path_enable(void *data)
{
	...
	ret = rt9466_i2c_test_bit(RT9466_REG_CHG_CTRL2, RT9466_SHIFT_CFO_EN);
	if (ret < 0)
		return ret;
...
static void rt9466_charger_intf_init(void)
{
	...
	mtk_charger_intf[CHARGING_CMD_SET_IRCMP_RESISTOR] = rt_charger_set_ircmp_resistor;
	mtk_charger_intf[CHARGING_CMD_SET_IRCMP_VOLT_CLAMP] = rt_charger_set_ircmp_vclamp;
