...
unsigned int g_fan53526_logger;
unsigned int g_fan53526_dbgaddr;
...
/*----------------debug machanism-----------------------*/
unsigned int fan53526_logger_level_set(unsigned int level, unsigned int addr)
{
	g_fan53526_logger = level >= FAN53526LOGLV ? level : FAN53526LOGLV;
	g_fan53526_dbgaddr = addr;
	return 0;
}

static ssize_t fan53526_logger_write(struct file *file, const char __user *buf, size_t size, loff_t *ppos)
{
	...
	fan53526_logger_level_set(dbg_lv_value, reg_addr);

	pr_err(PMICTAG "fan53526_logger = %d\n", g_fan53526_logger);
	pr_err(PMICTAG "fan53526_dbg_addr = 0x%x\n", g_fan53526_dbgaddr);
...
static ssize_t fan53526_dump_write(struct file *file, const char __user *buf, size_t size, loff_t *ppos)
{
	fan53526_dump_register();
	return size;
}

static int fan53526_logger_show(struct seq_file *s, void *unused)
{
	seq_puts(s, "usage: echo dbg_lv addr > fan53526_logger\n");
	seq_puts(s, "     dvb_lv: 1-7, default=4\n");
	seq_puts(s, "     addr: addr need to monitor\n");
	seq_printf(s, "fan53526_logger  = %d\n", g_fan53526_logger);
	seq_printf(s, "fan53526_dbg_addr = 0x%x\n", g_fan53526_dbgaddr);
	return 0;
}

static int fan53526_logger_open(struct inode *inode, struct file *file)
{
	return single_open(file, fan53526_logger_show, NULL);
}

static int fan53526_dump_show(struct seq_file *s, void *unused)
{
	seq_puts(s, "usage: echo > fan53526_dump_all_reg\n");
	return 0;
}

static int fan53526_dump_open(struct inode *inode, struct file *file)
{
	return single_open(file, fan53526_dump_show, NULL);
}

static const struct file_operations fan53526_logger_op = {
	.open    = fan53526_logger_open,
	.read    = seq_read,
	.write   = fan53526_logger_write,
	.llseek  = seq_lseek,
	.release = single_release,
};

static const struct file_operations fan53526_dump_op = {
	.open    = fan53526_dump_open,
	.read    = seq_read,
	.write   = fan53526_dump_write,
	.llseek  = seq_lseek,
	.release = single_release,
};

static int __init fan53526_debugfs_init(void)
{
	debugfs_create_file("fan53526_logger", S_IFREG | S_IRUGO, NULL, NULL, &fan53526_logger_op);
	debugfs_create_file("fan53526_dump_all_reg", S_IFREG | S_IRUGO, NULL, NULL, &fan53526_dump_op);

	return 0;
}

subsys_initcall(fan53526_debugfs_init);
/*----------------debug machanism-----------------------*/
...
static int fan53526_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	PMICLOG1("[fan53526_driver_probe] success\n");
	return err;
}
