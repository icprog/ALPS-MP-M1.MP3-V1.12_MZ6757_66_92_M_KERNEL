					...
					unsigned long event, void *data)
{
...
#ifdef CONFIG_MTK_PUMP_EXPRESS_PLUS_30_SUPPORT
		pr_err("TCP_NOTIFY_SINK_VBUS=> vol:%d cur:%d isCableIn:%d", pd_sink_voltage, pd_sink_current
			, isCableIn);

		if (pd_sink_voltage == 0) {
			if (isCableIn != false) {
				isCableIn = false;
				chrdet_int_handler();
			}
	...
	case TCP_NOTIFY_TYPEC_STATE:
		if (noti->typec_state.new_state == TYPEC_ATTACHED_SNK) {
			...
			typec_switch_usb_disconnect(NULL);
			mutex_unlock(&param_lock);
	...
	case TCP_NOTIFY_PD_STATE:
		...
		if (mtk_check_pe_ready_snk() == true)
			wake_up_bat3();
