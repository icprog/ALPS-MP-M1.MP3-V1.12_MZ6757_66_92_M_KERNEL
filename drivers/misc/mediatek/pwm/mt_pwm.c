...
static ssize_t pwm_debug_show(struct device *dev, struct device_attribute *attr, char *buf)
{
#ifdef TESE_MEM_MODE
	struct pwm_spec_config conf;
	unsigned long fifo1_val, fifo2_val;
	dma_addr_t phys;
	unsigned int *virt;
	int cnt_alloc_4g = 0;
	...
	for (cnt_alloc_4g = 0; cnt_alloc_4g < 1000; cnt_alloc_4g++) {
		virt = dma_alloc_coherent(dev, 2, &phys, GFP_KERNEL);
		if (phys < 0xffffffff) {
			PWMDBG("We get NOT large dram address !!!%lx\n", (unsigned long)phys);
			break;
		} else if (cnt_alloc_4g < 999) {
			dma_free_coherent(dev, 2, virt, phys);
		}
	}
	PWMDBG("We get address %lx\n", (unsigned long)phys);
	membuff = virt;
	membuff[0] = 0xaaaaaaaa;
	membuff[1] = 0xffff0000;
	conf.PWM_MODE_MEMORY_REGS.BUF0_SIZE = 1;
	conf.PWM_MODE_MEMORY_REGS.BUF0_BASE_ADDR = phys;
	pwm_set_spec_config(&conf);
	PWMDBG("Start wave\n");
#endif /*TESE_MEM_MODE*/
