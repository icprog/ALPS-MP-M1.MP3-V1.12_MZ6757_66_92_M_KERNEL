...
static int atm_hrtimer_polling_delay = TS_MS_TO_NS(TZCPU_ATM_HRTIMER_POLLING_DELAY); /* default 20ms*/
...
#if KPIH_ATM_PROFILE
static s64 latest_atm_loop_latency; /* us */
static s64 latest_atm_cpu_limit_lateny; /* us */
static s64 latest_atm_gpu_limit_lateny; /* us */

static s64 latest_atm_loop_latency2; /* us */
static int latest_atm_cpu_flg; /* us */
static int latest_atm_gpu_flg; /* us */
#endif
/*=============================================================
...
#if KPIH_ATM_PROFILE
s64 atm_get_latest_latency(void)
{
	return latest_atm_loop_latency;
}
EXPORT_SYMBOL(atm_get_latest_latency);

s64 atm_get_cpu_limit_latest_latency(void)
{
	return latest_atm_cpu_limit_lateny;
}
EXPORT_SYMBOL(atm_get_cpu_limit_latest_latency);

s64 atm_get_gpu_limit_latest_latency(void)
{
	return latest_atm_gpu_limit_lateny;
}
EXPORT_SYMBOL(atm_get_gpu_limit_latest_latency);
#endif
/**
...
static void set_adaptive_cpu_power_limit(unsigned int limit)
{
	...
	latest_atm_cpu_limit_lateny = 0;
	latest_atm_cpu_flg = 0;
#endif
	...
	if (prv_adp_cpu_pwr_lim != adaptive_cpu_power_limit) {
		...
		latest_atm_cpu_limit_lateny = ktime_to_us(delta);
		tscpu_warn("atmKPI limitcpu=%lld\n", latest_atm_cpu_limit_lateny);
		latest_atm_cpu_flg = 1;
#endif
	}
}
...
static void set_adaptive_gpu_power_limit(unsigned int limit)
{
	...
	latest_atm_gpu_limit_lateny = 0;
	latest_atm_gpu_flg = 0;
#endif
...
#if KPIH_ATM_PROFILE
		delta = ktime_sub(ktime_get(), now);
		latest_atm_gpu_limit_lateny = ktime_to_us(delta);
		tscpu_warn("atmKPI limitgpu=%lld\n", latest_atm_gpu_limit_lateny);
		latest_atm_gpu_flg = 1;
#endif
	}
}
...
#if KPIH_ATM_PROFILE
		delta = ktime_sub(ktime_get(), now);
		latest_atm_loop_latency = ktime_to_us(delta);
#endif
#endif
	}
	return 0;
...
static enum hrtimer_restart atm_loop(struct hrtimer *timer)
{
	...
	temp = snprintf(buffer, sizeof(buffer), "%s c %d p %d l %d ", __func__, atm_curr_maxtj, atm_prev_maxtj,
...
static int krtatm_thread(void *arg)
{
	...
	for (;;) {
...

		{
			...
			delta = ktime_sub(ktime_get(), now);
			latest_atm_loop_latency = ktime_to_us(delta);
			tscpu_warn("atmKPI_loop cpu=%d gpu=%d loopcpu=%lld loopgpu=%lld loop=%lld\n",
			latest_atm_cpu_flg, latest_atm_gpu_flg, latest_atm_cpu_limit_lateny,
				latest_atm_gpu_limit_lateny, latest_atm_loop_latency);

			if (latest_atm_cpu_flg == 1) {
				tscpu_warn("atmKPI_loop loop=%lld WITHcpu=%lld\n",
					latest_atm_loop_latency, latest_atm_cpu_limit_lateny);
				latest_atm_cpu_flg = 0;
				latest_atm_cpu_limit_lateny = 0;
			}
			if (latest_atm_gpu_flg == 1) {
				tscpu_warn("atmKPI_loop loop=%lld WITHgpu=%lld\n",
					latest_atm_loop_latency, latest_atm_gpu_limit_lateny);
				latest_atm_gpu_flg = 0;
				latest_atm_gpu_limit_lateny = 0;
			}
			if (latest_atm_cpu_flg == 0) {
				tscpu_warn("atmKPI_loop loop=%lld NOcpu=%lld\n",
					latest_atm_loop_latency, latest_atm_gpu_limit_lateny);
			}
			if (latest_atm_gpu_flg == 0) {
				tscpu_warn("atmKPI_loop loop=%lld NOgpu=%lld\n",
					latest_atm_loop_latency, latest_atm_gpu_limit_lateny);
			}
#endif
