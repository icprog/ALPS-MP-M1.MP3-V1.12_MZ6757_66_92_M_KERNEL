...
int reg_mon_enable = 0;
int reg_mon_type = 0;
int reg_mon_check_type = 0;
...
int reg_mon_list_i = 0;
int reg_mon_list_wrapped = 0;
				 ...
				 const char *buf, size_t size)
{
	...
	if (3 == sscanf(buf, "%x %x %x", &addr, &mask, &val))
				  ...
				  const char *buf, size_t size)
{
	...
	if (3 == sscanf(buf, "%d %d %d", &en, &type, &check_type))
		pr_debug("good format: '%s'", buf);
	else
		pr_debug("invalid format: '%s'", buf);
