...
void __iomem *CHN0_EMI_BASE_ADDR = NULL;
void __iomem *CHN1_EMI_BASE_ADDR = NULL;
void __iomem *DRAMC_CH0_TOP0_BASE_ADDR = NULL;
void __iomem *DRAMC_CH1_TOP0_BASE_ADDR = NULL;
...
static ssize_t emi_mpu_show(struct device_driver *driver, char *buf)
{
	...
	for (region = 0; region < EMI_MPU_REGION_NUMBER; region++) {
		start = (mt_emi_reg_read(EMI_MPU_SA(region)) << 16) + emi_physical_offset;
		end = (mt_emi_reg_read(EMI_MPU_EA(region)) << 16) + emi_physical_offset;
		ptr += sprintf(ptr, "R%d-> 0x%x to 0x%x\n", region, start, end+0xFFFF);
