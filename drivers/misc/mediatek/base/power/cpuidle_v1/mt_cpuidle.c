...
#ifdef CONFIG_MTK_RAM_CONSOLE
#define DORMANT_LOG(cid, idx) (sleep_aee_rec_cpu_dormant_va[cid] |= (1 << idx))
#define DORMANT_LOG_CLEAR(cid) (sleep_aee_rec_cpu_dormant_va[cid] = 0)
#else
#define DORMANT_LOG(cid, idx)
#define DORMANT_LOG_CLEAR(cid)
#endif
...
int mt_cpu_dormant_psci(unsigned long flags)
{
	...
	if (psci_ops.cpu_suspend) {
		DORMANT_LOG(clusterid * MAX_CORES + cpuid, 3);
...
int mt_cpu_dormant_reset(unsigned long flags)
{
	...
	DORMANT_LOG(clusterid * 4 + cpuid, 3);

	wfi();

	smp();

	DORMANT_LOG(clusterid * 4 + cpuid, 4);

	__enable_dcache();

	DORMANT_LOG(clusterid * 4 + cpuid, 5);
...
int mt_cpu_dormant(unsigned long flags)
{
	...
	cpuid = clusterid * MAX_CORES + cpuid;
