...
static disp_session_vsync_config vsyncConfig;
...
static int task_sync_handler(void *arg)
{
	...
	do {
		...
		if (VAL_TRUE == task_sync_data->bIsAudioFollowVsync) {

			primary_display_wait_for_vsync(&vsyncConfig);
...
void task_sync_step(void)
{
	if (VAL_FALSE == bVDecStarted) {
		...
		if (VAL_FALSE == taskSyncData.bIsAudioFollowVsync) {
...
{
	if (VAL_TRUE == taskSyncData.bIsAudioFollowVsync) {
	...
	if (rHWLock.eDriverType == VAL_DRIVER_TYPE_MP4_DEC ||
		...
		while (bLockedHW == VAL_FALSE) {
			...
			if (grVcodecDecHWLock.pvHandle == 0) { /* No one holds dec hw lock now */
				...
				/* task_sync_step(); Disable av task sync */
...
static long vcodec_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case VCODEC_DEC_DEC_EMI_USER:
	{
		...
		/* task_sync_stop(); Disable av task sync */
		mutex_unlock(&DecEMILock);
