...
static int DSI_Stop(DISP_MODULE_ENUM module, cmdqRecHandle cmdq)
{
	int i;
		     ...
		     unsigned char *para_list, unsigned char force_update)
{
	...
	memset(&t0, 0, sizeof(DSI_VM_CMD_CON_REG));
	memset(&t2, 0, sizeof(DSI_VM_CMD_CON_REG));
	/* DISPFUNC(); */
	for (d = DSI_MODULE_BEGIN(module); d <= DSI_MODULE_END(module); d++) {
...
UINT32 PanelMaster_get_CC(UINT32 dsi_idx)
{
	...
	memset(&tmp_reg, 0, sizeof(DSI_TXRX_CTRL_REG));
