...
unsigned int interrupts_size = ARRAY_SIZE(mt6336_interrupts);
/* prior irq CC & PD irq index, CC & PD have the same i index */
unsigned char cc_pd_i = INT_TYPE_C_CC_IRQ / CHR_INT_WIDTH;
unsigned char cc_j = INT_TYPE_C_CC_IRQ % CHR_INT_WIDTH;
unsigned char pd_j = INT_TYPE_C_PD_IRQ % CHR_INT_WIDTH;

/*****************************************************************************
 * MT6336 Interrupt service
 ******************************************************************************/
static DEFINE_MUTEX(mt6336_mutex);
struct task_struct *mt6336_thread_handle = NULL;
...
void mt6336_disable_interrupt(MT6336_IRQ_ENUM intNo, char *str)
{
	...
	if (shift >= ARRAY_SIZE(mt6336_interrupts)) {
		PMICLOG("[mt6336_disable_interrupt] fail intno=%d \r\n", intNo);
		return;
	}
...
{
	unsigned char i, j;
	unsigned char int_status_vals[interrupts_size];
	unsigned short cc_pd_status;
	unsigned int ret;

	ret = mt6336_read_bytes(mt6336_interrupts[0].address, int_status_vals, interrupts_size);
