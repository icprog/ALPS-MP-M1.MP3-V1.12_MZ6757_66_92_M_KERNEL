...
void aee_rr_init_thermal_temp(int num)
{
...
void aee_rr_rec_thermal_temp(int index, s8 val)
{
	if (!ram_console_init_done || !ram_console_buffer)
		return;
...
{
	seq_printf(m, "ptp_temp: 2L = %llx\n", LAST_RRR_VAL(ptp_temp) & 0xFF);
	seq_printf(m, "ptp_temp: L = %llx\n", (LAST_RRR_VAL(ptp_temp) >> 8) & 0xFF);
	seq_printf(m, "ptp_temp: CCI = %llx\n", (LAST_RRR_VAL(ptp_temp) >> 16) & 0xFF);
	seq_printf(m, "ptp_temp: GPU = %llx\n", (LAST_RRR_VAL(ptp_temp) >> 24) & 0xFF);
}
