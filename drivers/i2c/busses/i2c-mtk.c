...
static unsigned int aee_timeout_cnt;
...
static void dump_gpio_reg(struct mt_i2c *i2c)
{
	if (of_device_is_compatible(i2c->dev->of_node, "mediatek,mt6757-i2c")) {
		if (i2c->id == 1) {
			gpio_dump_reg(43);
			gpio_dump_reg(94);
			gpio_dump_reg(91);
...
static int mt_i2c_do_transfer(struct mt_i2c *i2c)
{
	...
	previous_addr = i2c->previous_addr;
	i2c->previous_addr = i2c->addr;

	tmo = wait_event_timeout(i2c->wait, i2c->trans_stop, tmo);

	if (tmo == 0) {
		dev_err(i2c->dev, "addr: %x, prev: %x, transfer timeout\n", i2c->addr, previous_addr);
		i2c_dump_info(i2c);
		dump_gpio_reg(i2c);
		mt_irq_dump_status(i2c->irqnr);
		dump_cg_regs();
		mt_i2c_init_hw(i2c);
		addr_reg = i2c_readw(i2c, OFFSET_SLAVE_ADDR) >> 1;
		if (i2c->addr == addr_reg)
			aee_timeout_cnt++;

		if (aee_timeout_cnt == 5)
			aee_kernel_warning("I2C", "I2C transfer timeout");
