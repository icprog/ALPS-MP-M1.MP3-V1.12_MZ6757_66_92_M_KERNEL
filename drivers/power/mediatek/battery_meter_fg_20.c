...
/* ============================================================ // */
int battery_meter_get_bif_battery_temperature(int *data)
{
	int bat_temperature_val = 0;
	int ret;

	ret = battery_charging_control(CHARGING_CMD_GET_BIF_TBAT, &bat_temperature_val);
	if (ret >= 0)
		*data = bat_temperature_val;

	return ret;
}

int battery_meter_get_bif_battery_voltage(int *data)
{
	unsigned int vbat = 0;
	int ret;

	ret = battery_charging_control(CHARGING_CMD_GET_BIF_VBAT, &vbat);
	if (ret >= 0)
		*data = vbat;
