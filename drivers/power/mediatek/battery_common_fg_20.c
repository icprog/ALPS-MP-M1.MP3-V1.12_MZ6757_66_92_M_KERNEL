...
#if defined(CONFIG_MTK_PUMP_EXPRESS_PLUS_20_SUPPORT) || defined(CONFIG_MTK_PUMP_EXPRESS_PLUS_30_SUPPORT)
#ifndef PUMP_EXPRESS_SERIES
#define PUMP_EXPRESS_SERIES
#endif
#endif
...
static ssize_t show_Pump_Express(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (STANDARD_CHARGER == BMT_status.charger_type &&
		...
		if (mtk_is_TA_support_pep30())
			is_ta_detected = 1;
		battery_log(BAT_LOG_CRTI, "%s: pep30_is_connect = %d\n",
			__func__, mtk_is_TA_support_pep30());
...
void bat_chrdet_int_handler(void)
{
	battery_log(BAT_LOG_CRTI, "[chrdet_int_handler]CHRDET status = %d....\n",
	...
	if (!pmic_get_register_value(PMIC_RGS_CHRDET)) {
		...
		if (boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT
			...
			battery_log(BAT_LOG_CRTI, "[chrdet_int_handler] Unplug Charger/USB\n");
			mt_power_off();
		}
	}
#endif
...
#ifdef REGISTER_CHRDET_INT_BY_BAT
	if (mtk_pep30_init() == false) {
		pmic_register_interrupt_callback(CHRDET_INT_NO, bat_chrdet_int_handler);
		pmic_enable_interrupt(CHRDET_INT_NO, 1, "battery");
	}
#endif
