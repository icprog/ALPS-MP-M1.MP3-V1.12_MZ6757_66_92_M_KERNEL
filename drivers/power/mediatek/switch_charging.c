...
int mtk_chr_enable_direct_charge(unsigned char charging_enable)
{
	int ret = 0;

	ret = battery_charging_control(CHARGING_CMD_ENABLE_DIRECT_CHARGE, &charging_enable);
	return ret;
}

int mtk_chr_enable_power_path(unsigned char en)
{
	u32 power_path_enable = 1;
	int ret = 0;

	power_path_enable = en;

	ret = battery_charging_control(CHARGING_CMD_ENABLE_POWER_PATH, &power_path_enable);
	return ret;
}

int mtk_chr_enable_charge(unsigned char charging_enable)
{
	int ret = 0;

	ret = battery_charging_control(CHARGING_CMD_ENABLE, &charging_enable);
	return ret;
}
...
PMU_STATUS BAT_BatteryStatusPEP30Action(void)
{
	battery_log(BAT_LOG_CRTI, "[BATTERY] BAT_BatteryStatusPE30Action !!\n\r");
	if (mtk_is_pep30_running() == false)
		BMT_status.bat_charging_state = CHR_CC;
...
void mt_battery_charging_algorithm(void)
{
	...
	if (mtk_pep30_check_charger() == true)
		BMT_status.bat_charging_state = CHR_PEP30;

	if (mtk_pdc_check_charger() == true) {
		mtk_pdc_init_table();
	}

	if (mtk_is_TA_support_pep30() == false) {
