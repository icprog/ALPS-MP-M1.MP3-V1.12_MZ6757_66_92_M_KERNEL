...
TRACE_EVENT(ppm_overutil_update,
	    ...
	    TP_STRUCT__entry(__field(int, ou_l)
			     __field(int, ou_h)
	    ),
	    TP_fast_assign(__entry->ou_l = overutil_l;
...
TRACE_EVENT(hps_update,
	    ...
	    TP_ARGS(actionID, online, cur_load, cur_tlp, cur_iowait, hvytsk, limit, base,
		    up_avg, down_avg, tlp_avg, rush_cnt, up_times, down_times, power_mode, target),
	    TP_STRUCT__entry(__field(unsigned int, actionID)
			     ...
			     __field(unsigned int, up_times)
			     __field(unsigned int, down_times)
			     __field(unsigned int, power_mode)
	    ...
	    TP_fast_assign(__entry->actionID = actionID;
			   ...
			   __entry->up_times = up_times;
			   __entry->down_times = down_times;
			   __entry->power_mode = power_mode;
			   __assign_str(target, target);),
	    TP_printk("(0x%X)%s action end (%u)(%u)(%u) %s %s%s (%u)(%u)(%u)(%u) [%d %d %d] %s",
		      ...
		      __entry->up_times, __entry->down_times, __entry->power_mode,
		      __get_str(target))
    );
