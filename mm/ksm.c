...
static int ksm_tuning_pressure_by_free_mem(void)
{
	struct zone *zone;
	static unsigned long ksm_threshold;
	...
	for_each_zone(zone) {
		...
		if (free_pages == 0)
			continue;
		if (ksm_threshold == 0 && zone->watermark[WMARK_HIGH] != 0)
			ksm_threshold = (zone->watermark[WMARK_HIGH]);
		if (free_pages < ksm_threshold)
			return 1;
	}
	return 0;
}
...
static void ksm_tuning_pressure(void)
{
	...
	if (bat_is_charger_exist() == KAL_TRUE) {
		...
		if (num_cpus >= three_quater_cpus) {
			...
			if (ksm_tuning_pressure_by_free_mem()) {
				ksm_thread_sleep_millisecs = 10000;
				ksm_thread_pages_to_scan = 200;
			} else {
				ksm_thread_sleep_millisecs = 20000;
				ksm_thread_pages_to_scan = 100;
			}
		}
	}
