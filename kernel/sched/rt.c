...
int rt_throttled(const struct cpumask *cpu_mask)
{
	int cpu;
	struct rq *rq;
...
int rt_timer_on(int dying_cpu)
{
	struct rq *rq;
	struct rt_bandwidth *rt_b;

	rq = cpu_rq(dying_cpu);
	rt_b = &rq->rt.tg->rt_bandwidth;

	if (hrtimer_active(&rt_b->rt_period_timer) &&
		(rt_b->rt_period_timer.base)->cpu_base == &per_cpu(hrtimer_bases, dying_cpu))
		return 1;

	return 0;
}

void restart_rt_timer_on(int dying_cpu, int new_cpu)
{
	struct rq *rq;
	struct rt_bandwidth *rt_b;
	unsigned long flag;
	...
	raw_spin_lock_irqsave(&rt_b->rt_runtime_lock, flag);
	start_bandwidth_timer(&rt_b->rt_period_timer, rt_b->rt_period);
	raw_spin_unlock_irqrestore(&rt_b->rt_runtime_lock, flag);
}
